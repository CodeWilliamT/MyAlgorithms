...
namespace ToolKits.HTCamera
{
    public class BaslerFromNIToHalcon : BaseCamera
    {
        ...
        private Int32 timeOut = 500;//ms
        private HObject image = new HObject();
        private Int32 img_width, img_height;
        ...
        public override bool OpenCamera()
        {
            try
            {
                ...
                if (err != CWIMAQdx.Error.Success) return false;
                Int32 triggerSource = (this.isSoftwareTrigger) ? 0 : 1;    //0:软触发;1:硬触发
                ...
                Int32 len = img_width * img_height;
                buffer = new byte[len];
        ...
        public override bool ChangeTriggerSource(bool isSoftwareTrigger)
        {
            ...
            //0:软触发
            //1:硬触发
            Int32 triggerSource = isSoftwareTrigger ? 0 : 1;
            try
            {
                if (isConnected)
                {
        ...
        /// </summary>
        /// <returns></returns>
        public override Acquisition Snap(Int32 imageNum, Int32 timeOut)
        {
    ...
    public class BaslerToHalcon : BaseCamera
    {
        ...
        /// <returns></returns>
        public override Acquisition Snap(Int32 imageNum = 1, Int32 timeOut = -1)
        {
        ...
        public override Acquisition GetFrames(Int32 imageNum = 1, Int32 timeOut = -1)
        {
            Acquisition _acq = new Acquisition();
            try
            {
        ...
        public override Task<Acquisition> GetFramesAsync(Int32 imageNum = 1, Int32 timeOut = -1)
        {
            return Task.Run(() =>
            {
    ...
    public class BaslerMtxCamLinkToHalcon : BaseCamera
    {
        ...
        private const Int32 FUNCTION_SUPPORTED_IMAGE_TYPE = (8 + MIL.M_UNSIGNED);
        private const Int32 BUFFERING_SIZE_MAX = 400;
        ...
        public override bool ClearImageQueue()
        {
            try
            {
                ...
                num = 0;
                for (Int32 j = 0; j < buffers.Length; j++)
                {
        ...
        public override Acquisition Snap(Int32 imageNum = 1, Int32 timeOut = -1)
        {
            ...
            try
            {
                ...
                for (Int32 i = 0; i < buffers.Length; i++)
                {
                    if (DateTime.Now.Subtract(t1).TotalMilliseconds > timeOut)//采集超时500ms
                    {
                        for (Int32 j = i; j < buffers.Length; j++)
                        {
                            if (buffers[j] == MIL.M_NULL) break;
                            MIL.MbufClear(buffers[j], 0);
                            buffers[j] = MIL.M_NULL;
    ...
    public class MultiCamToHalcon : BaseCamera
    {
        #region 相机自带函数
        /// <summary>
        ...
        private static void ThrowOnMultiCamError(Int32 status, String action)
        {
            if (status != 0)
            {
                ...
                throw new MultiCamToHalcon.MultiCamException(error);
            }
        }
        #endregion
        ...
        #endregion
        #region 配置参数
        MultiCamToHalcon.CALLBACK multiCamCallback;
        private string sn = "";
        private Camera camera = null;
        private string camFile = "beA4000-62km_P62RG";//"acA2040-180kc_P180SC";
        private string pixelType = EuresysPixelFormat.Y8;
        private bool isSoftwareTrigger = true;
        private bool isMirrorX = false;
        private bool isMirrorY = false;
        System.Collections.Concurrent.ConcurrentQueue<Acquisition> acqQueue = new System.Collections.Concurrent.ConcurrentQueue<Acquisition>();
        private const Int32 MAX_BUFFER_NUM = 400;
        IntPtr[] buffers = new IntPtr[MAX_BUFFER_NUM];
        private Int32 num = 0;
        ...
        #endregion
        public MultiCamToHalcon(string serialNumber)
        {
            this.camFile = "beA4000-62km_P62RG";
            this.sn = serialNumber;
        }
        public override bool InitCamera()
        {
            try
            {
                for (Int32 i = 0; i < MAX_BUFFER_NUM; i++)
                {
                    buffers[i] = (IntPtr)0;
                }
                if (camera == null)
        ...
        public override bool OpenCamera()
        {
            try
            {
                ...
                MultiCamToHalcon.OpenDriver();
                isSoftwareTrigger = false;
                // Enable error logging
                MultiCamToHalcon.SetParam(MultiCamToHalcon.CONFIGURATION, "ErrorLog", "error.log");

                // Create a channel and associate it with the first connector on the first board

                MultiCamToHalcon.SetParam(MultiCamToHalcon.BOARD + 0, "BoardTopology", "MONO_DECA");
                MultiCamToHalcon.Create("CHANNEL", out channel);
                //不同板卡设置不同索引？
                MultiCamToHalcon.SetParam(channel, "DriverIndex", 0);
                // For all Domino boards except Domino Symphony
                MultiCamToHalcon.SetParam(channel, "Connector", "M");
                // Choose the CAM file
                MultiCamToHalcon.SetParam(channel, "CamFile", camFile);
                // Choose the pixel color format
                MultiCamToHalcon.SetParam(channel, "ColorFormat", pixelType);

                // Choose the way the first acquisition is triggered
                MultiCamToHalcon.SetParam(channel, "AcquisitionMode", "HFR");
                MultiCamToHalcon.SetParam(channel, "TrigMode", "HARD");
                MultiCamToHalcon.SetParam(channel, "NextTrigMode", "SAME");
                // Choose the number of images to acquire
                MultiCamToHalcon.SetParam(channel, "SeqLength_Fr", MultiCamToHalcon.INDETERMINATE);
                MultiCamToHalcon.SetParam(channel, "TrigCtl", "ISO");
                MultiCamToHalcon.SetParam(channel, "TrigLine", "IIN1");

                MultiCamToHalcon.GetParam(channel, "ImageSizeX", out width);
                MultiCamToHalcon.GetParam(channel, "ImageSizeY", out height);
                MultiCamToHalcon.GetParam(channel, "BufferPitch", out bufferPitch);

                // Register the callback function
                multiCamCallback = new MultiCamToHalcon.CALLBACK(MultiCamCallback);
                MultiCamToHalcon.RegisterCallback(channel, multiCamCallback, channel);

                // Enable the signals corresponding to the callback functions
                MultiCamToHalcon.SetParam(channel, MultiCamToHalcon.SignalEnable + MultiCamToHalcon.SIG_SURFACE_PROCESSING, "ON");
                MultiCamToHalcon.SetParam(channel, MultiCamToHalcon.SignalEnable + MultiCamToHalcon.SIG_ACQUISITION_FAILURE, "ON");
                ...
                camera.Parameters[PLCamera.ClPixelClockAbs].SetValue(64500000);
                camera.Parameters[PLCamera.ClTapGeometry].SetValue("Geometry1X10_1Y");
                MultiCamToHalcon.SetParam(channel, "ChannelState", "ACTIVE");
                return true;
            }
        ...
        public override bool ChangeTriggerSource(bool isSoftwareTrigger)
        {
            ...
            if (isSoftwareTrigger)
            {
                MultiCamToHalcon.SetParam(channel, "ChannelState", "IDLE");
                MultiCamToHalcon.SetParam(channel, "TrigMode", "SOFT");
                if (camera.IsOpen)
                {
                    camera.Close();
                    Thread.Sleep(50);
                ...
                camera.Open();
                camera.Parameters[PLCamera.TriggerSelector].SetValue("AcquisitionStart");
        ...
        public override Acquisition Snap(Int32 imageNum = 1, Int32 timeOut = -1)
        {
            ...
            try
            {
                timeOut = (timeOut == -1) ? 100000 : timeOut;
                String channelState;
                MultiCamToHalcon.GetParam(channel, "ChannelState", out channelState);
                if (channelState != "ACTIVE")
                    MultiCamToHalcon.SetParam(channel, "ChannelState", "ACTIVE");
                // Generate a soft trigger event
                MultiCamToHalcon.SetParam(channel, "ForceTrig", "TRIG");
                DateTime t1 = DateTime.Now;
                        ...
                        if (buffers[i].ToInt32() != 0)
                        {
                            ...
                            unsafe
                            {
                                //lock (this)
                                {
                                    ...
                                    {
                                        //黑白图像
                                        HOperatorSet.GenImage1(out img, "byte", Convert.ToInt32(width), Convert.ToInt32(height), buffers[i]);
                                        //HOperatorSet.GenImage1Rect(out img, ptr, Convert.ToInt32(imageSizeX), Convert.ToInt32(imageSizeY), Convert.ToInt32(SrcImagePitchByte), 8, 8, "true", 0);
                                    ...
                                    {
                                        //RGB彩色图像
                                        HOperatorSet.GenImageInterleaved(out img, buffers[i], "rgb", width + 2, height, -1, "byte", 0, 0, 0, 0, -1, 0);
                                    }
                                    else
                                    {
                                        img.Dispose();
        ...
        private void ProcessingCallback(SIGNALINFO signalInfo)
        {
            ...
            MultiCamToHalcon.GetParam(currentSurface, "SurfaceAddr", out bufferAddress);
            buffers[num] = bufferAddress;
            num++;
        }
    ...
    public class MVSCamToHalcon : BaseCamera
    {
        ...
        public override bool OpenCamera()
        {
            try
            {
                ...
                m_pOperator.SetBoolValue("LineInverter", false);
                ChangeTriggerLight(true);
            ...
            try
            {
                if (isTrggerLight)
                {
                    if (m_pOperator.SetEnumValue("LineSelector", 1) != 0)
                        return false;
                    if (m_pOperator.SetEnumValue("LineMode", 8) != 0)
                        return false;
                }
                else
                {
