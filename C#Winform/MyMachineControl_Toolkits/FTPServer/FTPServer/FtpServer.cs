...
namespace FTPServer
{
    ...
    public class FtpServer
    {
        ...
        /// <param name="workDir">作为存储目录的物理路径</param>
        /// <param name="addInfo">状态刷新方法</param>
        public FtpServer(string ip, int port, string workDir,AddInfoDelegate addInfo)
        {
            this.ip = ip;
            this.port = port;
            this.workDir = workDir;
            this.addInfo=new AddInfoDelegate(addInfo);
            AddUser("admin", "admin");
        }
        // 向屏幕输出显示状态信息的方法
        ...
        /// 获取与设置服务器是否已链接地址
        /// </summary>
        public bool IsConnect { get { return isConnect; } set { this.isConnect = value; } }

        /// <summary>
        /// 获取与设置服务器物理目录
        /// </summary>

        public string WorkDir{ get { return workDir; } set { this.workDir = value; } }
        ...
        /// 开始或停止FTP服务器
        /// </summary>
        /// <returns>true为开始成功，false为停止成功</returns>
        public bool StartStopFtpServer()
        {
        ...
        private void CommandNLST(User user, string parameter)
        {
            ...
            for (int i = 0; i < dir.Length; i++)
            {
                ...
                // 按下面的格式输出目录列表
                sendString +=  folderName + Environment.NewLine;
            }
            ...
            for (int i = 0; i < files.Length; i++)
            {
                ...
                // 按下面的格式输出文件列表
                sendString +=fileName + Environment.NewLine;
            }
