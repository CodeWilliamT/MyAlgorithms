...
namespace LeadframeAOI
{
    ...
    {
        public int Num;
        public ConcurrentDictionary<int,Box> Dir_Boxes;
        ...
        public bool Save()
        {
            string path = App.SystemDir;
            IniFiles config = new IniFiles(path + "\\BoxData.ini");
            int i = 0;
            foreach(var pair in Dir_Boxes)
            {
                Box item = pair.Value;
                config.SaveObj(item, "Box_" + i);
                i++;
            }
            Num = Dir_Boxes.Count;
        ...
        public bool Load()
        {
            string path = App.SystemDir;
            IniFiles config;
            if (File.Exists(path + "\\BoxData.ini"))
            {
                config = new IniFiles(path + "\\BoxData.ini");
                config.LoadObj(this);
                Dir_Boxes = new ConcurrentDictionary<int, Box>();
                Box BoxItem = null;
                for (int i = 0; i < Num; i++)
        ...
        {
            string[] ArrayStr = new string[App.boxManager.Dir_Boxes.Count];
            int i = 0;
