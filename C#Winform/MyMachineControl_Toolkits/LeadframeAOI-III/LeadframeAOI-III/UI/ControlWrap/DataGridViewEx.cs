...
namespace HTImageServer.UI.ControlWrap
{
    ...
    public partial class DataGridViewEx : DataGridView
    {
        ...
        {
            InitializeComponent();
            this.CellMouseLeave += new DataGridViewCellEventHandler(DataGridViewEX_CellMouseLeave);
        ...
        protected void DataGridViewEX_CellMouseLeave(object sender, DataGridViewCellEventArgs e)
        {
        ...
        protected void DataGridViewEX_CellMouseMove(object sender, DataGridViewCellMouseEventArgs e)
        {
            hoverCell.X = e.ColumnIndex;
            hoverCell.Y = e.RowIndex;
            if (hoverCell.X != -1 && hoverCell.Y != -1)
            {
        ...
        private void DrawCell(DataGridViewCellPaintingEventArgs e)
        {
            ...
            if (this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) && e.RowIndex != -1)
            {
                ...
                if (MergeColumnNames != null && this.MergeColumnNames.Contains(this.Columns[e.ColumnIndex].Name) && (hoverCell.X != -1 && hoverCell.Y != -1 && e.ColumnIndex == hoverCell.X && e.RowIndex == hoverCell.Y))
                {
                    using (SolidBrush brush = new SolidBrush(Color.Peru))
                    {
                        //以背景色填充
                        e.Graphics.FillRectangle(brush, e.CellBounds);

                        PaintingFont(e, cellwidth, 1, 0, 1);
                    }
                }
                else
                {
