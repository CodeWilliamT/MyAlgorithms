/****************************************************************************/
/*  File Name   :   Vision.cs                                               */
/*  Brief       :   Vision functions                                        */
/*  Date        :   2018/3/16                                               */
/*  Author      :   Weiliang TONG                                           */
/****************************************************************************/
using System;
...
namespace CameraZaxisScanModel
{
    ...
    {
        public Vision.AutoFocusParam focusPara;
        ...
        public void initial()
        {
            focusPara = new Vision.AutoFocusParam();
    ...
    public class Vision
    {
        ...
        private Acquisition acq;//图像存储内存对象
        ...
        public void InitializeAllCamera()
        {
            ...
            for (int i = 0; i < Obj_Camera.Num_Camera; i++)
            {
                ...
                {
                    MessageBox.Show("初始化相机"+i.ToString()+"失败！");
                    continue;
                }

                if (!obj_camera[i].OpenCamera())
                {
                    MessageBox.Show("打开相机" + i.ToString() + "失败！");
                    continue;
                }
                if (!Directory.Exists(obj_camera[i].cameraPath))
                {
                    Directory.CreateDirectory(obj_camera[i].cameraPath);
                }
            }
        }
        ...
        public void Initialize()
        {
            try
            {
                ...
                if (interactiveData.focusPara == null)
                {
                    interactiveData.focusPara = new Vision.AutoFocusParam();
                    interactiveData.focusPara.focusRegion = new HObject();
                }
                if (File.Exists(Application.StartupPath + "\\focusRegion.reg"))
                    HOperatorSet.ReadRegion(out interactiveData.focusPara.focusRegion, Application.StartupPath + "\\focusRegion.reg");
                Acq = new Acquisition();
                VisionState = 0;
        ...
        public void ShowImage(HTWindowControl htWindow, HObject image, HObject region)
        {
            ...
            {
                htWindow.BeginInvoke(new ShowImageDelegate(ShowImage), new object[] { htWindow, image, region });
            }
            else
            {
        ...
        public Boolean autoFocus(out Double bestFocus, HTWindowControl htWindow, CameraMgr cam, AutoFocusParam focusPara, Double timeOut = 1000)
        {
            ...
            for (int i = 0; i < acq.index + 1; i++)
            {
                if (htWindow != null)
                {
                    ...
                    Program.mainWindow.ShowImage(htWindow, imageshow, null);
                }
            }
            #endregion
            if (grabNum != imgNum)
        ...
        public Boolean matchModel(HObject image, Model model, out HTuple u, out HTuple v, out HTuple angle)
        {
            ...
            try
            {
                ...
                ToolKits.FunctionModule.Vision.find_model(image, matchRegion, model.showContour, out updateShowCont, model.modelType, model.modelID, -1, -1, model.scoreThresh,
                                                          ...
                                                          out hom2d, out iFlag);
                if (Program.mainWindow.htWindow != null)
                {
                    Program.mainWindow.htWindow.RefreshWindow(image, null, "fit");
                }
        ...
        public class Model
        {
            ...
            {
                if (!Visions.ClearModel(modelType, modelID))
                {
                    modelType = new HTuple();
                    modelID = new HTuple();
                    return false;
    ...
    public class DieInfo
    {
        ...
        public DieInfo(int r_index, int c_index, Double x_pos, Double y_pos, Double z_pos, Double u_pix, Double v_pix)
        {
            r = r_index;
            c = c_index;
            x = x_pos;
